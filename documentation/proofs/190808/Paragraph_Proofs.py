...
def textGenerator(textPath, fSize, goofNum, pLength, pageNum, fontPick=None, track=None, whatCase=None):
    ...
    for x in range(0, pLength, 1):    
        text.fontVariations(opsz = fSize , wght = pageNum, goof = goofNum )
...
def textDrawings(textPath, case = None, fontPicker = None):
    for g in (1,50,100):
        for y in range(100,1001,450):
            ...
            font(fnames[2], 8)
            text("Weight: %s, Goofy: %s" % (y, goofNumb), (50, height()-50))
...
textDrawings(textHamburgerfontsiv)
textDrawings(textHamburgerfontsiv, case = "upper")
textDrawings(textHamburgerfontsiv, case = "title")
