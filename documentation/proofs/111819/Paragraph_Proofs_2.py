...
for weight in (100,400,900):
    for opticalsz in (9.1, 18, 36, 144):
        for case in (None, "upper", "title"):
            newPage("TabloidLandscape")
            textBox(textGenerator2(textHamburgerfontsiv, opticalsz, 48, 400, weight, whatCase=case), (margin,margin, (width()-margin*2), (height()-margin*2)))
            font("Recur Mono", 9)
            text("OpSz: %s, Wght: %s" % (opticalsz,weight), (50,50))
