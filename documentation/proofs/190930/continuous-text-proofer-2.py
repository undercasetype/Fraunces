...
print(wghtMin, wghtMax, opMin, opMax, goofMin, goofMax)
...
for g in (0.1, 50, 100):
        ...
        if g == 100:
            section = "Goofy Max"
        font(fnames[0], 36)
        fontVariations(opsz = 36, wght = 400, WONK = 1, GOOF = 0.1)
        ...
        for x in (0.1, 400, 1000):
            for z in (9.1, 36, 144):
                for y in (0, 1):
                    ...
                    newPage("TabloidLandscape")
                    font(fnames[0], 20)
                    fontVariations(opsz = z, wght = x, WONK = y, GOOF = g)
                    ...
                    font(fnames[2], 10)
                    text("Weight: %s, Optical Size: %s, Wonk: %s, Goof: %s" % (x,z,y,g), (margin,margin))
